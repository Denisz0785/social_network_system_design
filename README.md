...
- RF 2-3 (в зависимости от средств)
...
```
 Hosts = 126 (HDD 8TB вообще нужно 115, но для iops нужно 126) / 2 = 63 
 Hosts_with_replication = 63 шардов * 2(RF) = 126 (тк такое кол-во дисков было выбрано из-за объема хранимых данных, то уменьшить кол-во хостов за счет реплик мы не можем)
```
- В связи с большим кол-м дисков необходимо шардирование (невозможно разместить на одном хосте)
- способ шардирования: Range based (нет потребности хранить все данные по пользователю отдельно, 
тк пользователи могут просматривать совместно одну и ту же инфо, и инфо друг о друге )

#### если рассчитывать кол-во хостов отдельно для подсистем: посты, комментарии, лайки и подписки, то:

- **по постам:** не будет отличаться от рассчетов представленных выше - 126 шардов с учетом RF 2

- **по комментариям:**
  - Disks_for_capacity(HDD 2TB) = 1TB/2TB ~= 1
  - Disks_for_throughput = (35кб + 3,5мб) = 3,5MB/2TB ~= 1  
  - Disks_for_iops = 120(запись) +11574(чтение) = 11694/100 = 117
  - Host = 117 / 2 = 59 hosts with 2 disk
  - Hosts_with_replication = 59 шардов * 2 = 118
...
- **по лайкам:** 
  - Disks_for_capacity(HDD 2TB) = 88GB/2TB ~= 1
  - Disks_for_throughput = 3,5KB/2TB ~= 1  
  - Disks_for_iops = 350/100 = 4
  - Host = 4 / 2 = 2 hosts with 2 disk
  - Hosts_with_replication = 2 шарда * 2 = 4  
  
- **по подпискам:** 
  - Disks_for_capacity(HDD 2TB) = 29GB/2TB ~= 1
  - Disks_for_throughput = 920byte/2TB ~= 1  
