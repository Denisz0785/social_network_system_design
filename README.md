...
rps: 1 000 000 * 20 / 86400 ~= 231 rps  
...
- подсистема Комментариев = 120 +231 = 351 / 100 ~= 4
...
- подсистема Комментариев = 4 (HDD 500ГБ)
...
```
   - способ шардирования: 
       - Range based для S3 (фото будут равномерно распределяться по шардам)
       - Key based для постов с хэшированием по user_id (чтобы все посты одного юзера хранились на одном шарде), так мы экономим на кросс-шардовых запросах
```
...
- **по комментариям:**
  ...
  - Hosts_with_replication = 2 шардов * 2(RF) = 4
  - тк 99% хостов необходимы для чтения, то кол-во хостов нужно в 2 раза меньше(при RF 2), тк часть нагрузки будет распределяться на реплики: 4 / 2 = 2 шардов  
